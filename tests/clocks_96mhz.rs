//! Clock configuration self-tests — validates 96 MHz overclock preset.
//!
//! Priority: HIGH
//! Wiring: None
//!
//! Uses `freeze_at(ClockSpeed::Mhz96)` to configure:
//!   96 MHz core, 48 MHz bus, 24 MHz flash
//! PLL: 16 MHz / 4 = 4 MHz ref × 24 = 96 MHz
...
mod tests {
    ...
    /// Core clock should be 96 MHz.
    /// Cross-validates the Clocks struct against MCG PLL register settings.
    /// PLL output = (16 MHz / (3+1)) * (0+24) = 4 * 24 = 96 MHz.
    #[test]
    fn test_core_clk_96mhz(state: &mut super::State) {
        defmt::assert_eq!(
        ...
        let vdiv = mcg.c6().read().vdiv0().bits() as u32;   // should be 0
        let pll_output = (16_000_000 / (prdiv + 1)) * (vdiv + 24);
        defmt::info!("PLL math: 16MHz / {} * {} = {} Hz", prdiv + 1, vdiv + 24, pll_output);
        defmt::assert_eq!(pll_output, 96_000_000, "PLL register math should yield 96 MHz");
    ...
    /// Bus clock should be 48 MHz (core / 2).
    #[test]
    fn test_bus_clk_48mhz(state: &mut super::State) {
        defmt::assert_eq!(
            state.clocks.bus_clk().raw(),
            48_000_000,
            "Bus clock should be 48 MHz"
        );
    ...
    /// Flash clock should be 24 MHz (core / 4).
    #[test]
    fn test_flash_clk_24mhz(state: &mut super::State) {
        ...
        defmt::assert_eq!(outdiv4, 3, "OUTDIV4 should be 3 (divide by 4)");
    }
